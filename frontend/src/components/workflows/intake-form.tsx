...
const EXAMPLES = ["Build a Task Management SaaS", "Build a Notes App", "Build a Marketplace for handmade goods"];
...
export function IntakeForm() {
  ...
  const handleSubmit = () => {
    ...
    submit.mutate(
      ...
      {
        onSuccess: (result) => {
          toast.success("Workflow started", { description: "Intent Engine → Business Analyst → Supervisor Brain…" });
          router.push(`/workflows/${result.workflowRunId}`);
        },
        onError: (err) => {
          toast.error("Failed to start workflow", { description: String(err) });
        },
      },
    );
  };
    ...
    <Card className="border-border/60 bg-card/60">
      <CardContent className="space-y-3">
        <div className="flex items-center gap-2 text-sm font-medium">
          <Sparkles className="h-4 w-4 text-status-running" />
          Describe what to build
        </div>
        <Textarea
          value={rawInput}
          onChange={(e) => setRawInput(e.target.value)}
          placeholder="Build a Task Management SaaS"
